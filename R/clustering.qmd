...
#Many incorrect
table(Cluster = kmeans_result$cluster, TrueClass = df_with_classes$class)
...
for (k in 2:10){ 
  ...
  df_temp <- tibble(
    ...
    classification = em_model$classification)
...
em_results %>% 
  ...
  ggplot(aes(x = k, y = silhouette))+
    geom_line()+
    geom_point(shape = 1, size = 2, stroke = 1, color = "white")+
...
ggsave(here::here('data/figures/4_em_clustering_assignment_w_silhouette.svg'), width = 130, height = 100, units = "mm")
```
...
ggsave(here::here('data/figures/5_select_dbscans.svg'), width = 200, height = 75, units = "mm")

#Plot all
for (plot in dbscan_plot_list){
  print(plot)
}

```

Make cluster 1 more dense
```{r}
set.seed(123)
...
ggsave(here::here('data/figures/6_density_heatmaps.svg'), width = 100, height = 100, units = "mm")
...
ggsave(here::here('data/figures/7_select_dbscans_dense.svg'), width = 200, height = 75, units = "mm")
```
